...
const NAV_LINKS = [
    { href: "/start", label: "Build Site", icon: Zap, description: "Launch marketing assets for any project" },
    { href: "/trending", label: "Chat Agent", icon: Bot, description: "AI Sales Agents for Instagram" },
    { href: "/discover", label: "Market Library", icon: Library, description: "3,750+ Real Estate Projects" },
      ...
      <div className="container h-full flex items-center justify-between px-4 md:px-6 max-w-[1800px]">
        ...
        <div className="flex items-center gap-3">
            <Link href="/dashboard" className="hidden sm:block">
                <Button className="h-10 px-6 rounded-full bg-white text-black font-bold text-[10px] uppercase tracking-widest hover:scale-105 transition-all shadow-xl shadow-white/10 group">
                    {user ? 'Dashboard' : 'Start Now'} <ArrowRight className="ml-2 h-3 w-3 group-hover:translate-x-1 transition-transform" />
                </Button>
            </Link>
            ...
            <button 
                ...
                className="lg:hidden p-2 rounded-xl bg-white/5 text-zinc-400 hover:text-white transition-all z-[120]"
            >
            ...
            >
                <div className="flex-1 overflow-y-auto px-6 pt-24 pb-12">
                    <div className="space-y-10">
                        <div>
                            <p className="text-[10px] font-bold text-zinc-600 uppercase tracking-widest mb-6 px-1">Main Menu</p>
                        ...
                        <div>
                            <p className="text-[10px] font-bold text-zinc-600 uppercase tracking-widest mb-6 px-1">Shortcuts</p>
                            <div className="space-y-3">
                                <Link href="/dashboard" className="block">
                                    <div className="p-6 rounded-[1.5rem] bg-blue-600 text-white shadow-xl shadow-blue-600/20 group">
                                        ...
                                        <h4 className="text-lg font-bold tracking-tight">Open Dashboard</h4>
                                        <p className="text-blue-100/60 text-[10px] font-bold uppercase tracking-widest">Manage everything</p>
                                    </div>
                                </Link>
                ...
                <div className="p-6 border-t border-white/5 bg-zinc-950/80 backdrop-blur-xl">
                    {!user ? (
                        <Link href="/start">
                            <Button className="w-full h-14 rounded-xl bg-white text-black font-bold text-sm uppercase tracking-widest shadow-xl">
                                Get Started Free
                            </Button>
                        </Link>
                    ) : (
                        <div className="flex items-center gap-3 bg-white/5 p-3 rounded-2xl border border-white/5">
                             ...
                             <Button variant="ghost" size="icon" className="h-8 w-8 text-zinc-600" onClick={() => auth.signOut()}>
                                <X className="h-4 w-4" />
                             </Button>
                        </div>
                    )}
