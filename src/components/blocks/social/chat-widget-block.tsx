...
import { MessageCircle, X, Send, Bot, Zap, Sparkles } from "lucide-react";
...
export function ChatWidgetBlock({
  ...
  const handleSend = () => {
      ...
      setTimeout(() => {
          setMessages(prev => [...prev, { role: 'agent', text: "Analyzing market data... Creek Harbour units are currently seeing a 7.4% rental yield. Would you like to see the available units?" }]);
      }, 800);
  };
        ...
        <div className="bg-zinc-950 border border-white/10 rounded-[2rem] shadow-2xl w-[380px] overflow-hidden animate-in slide-in-from-bottom-10 fade-in duration-500 flex flex-col h-[550px] backdrop-blur-3xl">
            ...
            <div className="bg-blue-600 p-6 flex items-center justify-between text-white relative">
                ...
                <div className="flex items-center gap-4 relative z-10">
                    ...
                    <div>
                        <h4 className="font-bold text-base leading-none mb-1">{agentName}</h4>
                ...
                <Button variant="ghost" size="icon" className="h-10 w-10 hover:bg-white/10 text-white rounded-full" onClick={() => setIsOpen(false)}>
                    <X className="h-5 w-5" />
                </Button>
            </div>
            ...
            <div className="p-6 bg-zinc-950 border-t border-white/5">
                <div className="flex items-center gap-3 bg-white/5 rounded-2xl p-2 border border-white/5 group focus-within:border-blue-500/50 transition-all">
                    <input 
                        ...
                        className="flex-1 bg-transparent border-0 focus:ring-0 text-sm text-white placeholder:text-zinc-600 px-3 h-10"
                        value={input}
                    ...
                    <button 
                        onClick={handleSend}
                        className="w-10 h-10 rounded-xl bg-blue-600 text-white flex items-center justify-center hover:scale-105 active:scale-95 transition-all shadow-lg shadow-blue-900/20"
                    >
