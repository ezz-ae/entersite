...
export default function TemplatesPage() {
  ...
  return (
    <main className="min-h-screen bg-background flex flex-col font-sans">
      ...
      {/* Hero */}
      <section className="bg-zinc-950 text-white border-b border-white/10 pt-32 pb-20">
...
function TemplateCard({ template }: { template: SiteTemplate }) {
    // Generate a visual preview based on template type
        ...
        <Card className="overflow-hidden hover:shadow-xl transition-all duration-300 group flex flex-col h-full border-border/50">
            <div className="relative aspect-[16/10] overflow-hidden bg-muted">
                ...
                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                    <Link href={`/builder?template=${template.id}`}>
                        <Button className="rounded-full font-bold shadow-lg transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300">
                            Use Template
                        </Button>
                    </Link>
                </div>
            ...
            <CardFooter className="p-5 pt-0 border-t border-border/40 mt-auto bg-muted/10">
                <Link href={`/builder?template=${template.id}`} className="w-full">
                    <Button variant="ghost" className="w-full justify-between hover:bg-background group/btn">
                        Start Building
